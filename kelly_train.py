...
#LEARNING_RATE = 0.00025#0.00001          # Set to 0.00025 in Pong for quicker results. 
...
def train():
    ...
    with tf.Session() as sess:
        ...
        while frame_number < MAX_FRAMES:
            ...
            saver.save(sess, PATH+'/'+RUNID, global_step=frame_number)
            frames_for_gif = []
...
def transfer_initialized_train_dqn(trained_path = None, save_file = None, model_name="my_model"):
    ...
    with tf.Session() as sess:
        ...
        if trained_path != None:
            ...
            optimistic_restore(sess, tf.train.latest_checkpoint(trained_path), [v for v in MAIN_DQN_VARS if "conv1" in v.name]+[v for v in TARGET_DQN_VARS if "conv1" in v.name])
...
if TRAIN_TRANSFER:
    ...
    transfer_initialized_train_dqn(trained_path = trained_path, 
                               save_file = save_file, 
                               model_name="transfer_learn_conv1_{}".format(ENV_NAME))
